if vim.loop.os_uname().sysname == "Windows_NT" then
  vim.keymap.set(
    { "t" },
    "<c-e><c-s>",
    [[<esc>python -c "import sys; print(sys.executable)"<enter>]]
  )
  vim.keymap.set({ "t" }, "<c-e><c-a>", [[<esc>.\.venv\Scripts\activate<enter>]])
  vim.keymap.set({ "t" }, "<c-e><c-d>", [[<esc>deactivate<enter>]])
else
  vim.keymap.set(
    { "t" },
    "<c-e><c-s>",
    [[<c-u>python3 -c "import sys; print(sys.executable)"<enter>]]
  )
  vim.keymap.set({ "t" }, "<c-e><c-a>", [[<c-u>./.venv/bin/activate<enter>]])
  vim.keymap.set({ "t" }, "<c-e><c-d>", [[<c-u>deactivate<enter>]])
end
